...
/**
  * A wrapper that handles the calls to the actual solr index
 */
class SolrIndex{
    ...
    public static function runCurlRequest($curl){
        ...
        if(!$out['error']){
            ...
            $out['headers'] = SolrIndex::getHeadersFromCurlResponse($out);
    ...
    public static function curlPostJson($uri, $json){
        $ch = SolrIndex::getCurlHandle($uri);
    ...
    public static function getBodyFromCurlResponse($response){
    ...
    public static function getSolrResponse($query){
        ...
        $response = SolrIndex::curlPostJson($solr_query_uri, json_encode($query));
